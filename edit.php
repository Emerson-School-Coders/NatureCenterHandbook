...
$page = $db->querySingle("SELECT title, author, entry, imageids FROM handbook WHERE id=".SQLite3::escapeString($_GET['id']), true);
$hidden = false;
?>
<html>
  <head>
  ...
  <body>
    ...
    else if (!$db->querySingle("SELECT canedit FROM passwords WHERE id=".SQLite3::escapeString($_COOKIE['userid']))) {$hidden = true; echo "<p>Sorry, you are not allowed to edit this page. Please ask Tim for permission.</p><!--";} ?>
    <center>
    <form action="edit_page.php" method="POST">
      ...
      <h3>Author: <?php echo $page['author']; ?>, edited by <?php echo $db->querySingle("SELECT name FROM passwords WHERE id=".SQLite3::escapeString($_COOKIE['userid'])); ?><br>
      ...
      <?php if (explode(",", $page["imageids"])[1] != "-1" && explode(",", $page["imageids"])[1] != "")
              echo '<br>Image 2: <img src="images/id-'.substr($page['imageids'], 6, 1).'" height=100> Change: <input type="file" name="image2" accept=".png, image/png, .jpg, image/jpg, .gif, image/gif">'; ?>
      <br><input type="submit" value="Submit">
    </form>
    <?php if (!$db->querySingle("SELECT candelete FROM passwords WHERE id=".SQLite3::escapeString($_COOKIE['userid']))) {$hidden = true; echo "<!--";} ?>
