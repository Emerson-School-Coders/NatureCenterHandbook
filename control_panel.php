    ...
    <div id="viewdeletions">
      ...
      <table>
        ...
        while ($delete = $deletions->fetchArray(SQLITE3_ASSOC)) {
          echo '<tr><td>'.$delete['title'].'</td><td>'.$delete['author'].'</td><td>'.$db->querySingle("SELECT name FROM passwords WHERE id=".$delete['removedby']).'</td><td>'.date('Y-m-d', $delete['removaldate']).'</td><td><form action="restore.php" method="GET"><input type="hidden" name="id" value="'.$page['id'].'"><input type="submit" value="Restore"></td><td><form action="delete.php" method="GET" onclick="var delete_page = confirm(\'Are you sure you want to delete this forever? It cannot be recovered!\');if(delete_page==true){window.location="delete.php?id=" . $page['id'] . ""}"><input type="hidden" name="id" value="'.$page['id'].'"><input type="submit" value="Remove" style="color: red"></td></tr>';
        }
        ?>
      </table>
    </div>
